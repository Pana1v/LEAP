...
NUM_BINS = 4
NUM_SCENARIOS = 5000
OBJECT_COUNTS = [3, 5, 10, 20, 40, 100, 200]
ILP_MAX_OBJECTS = 60  # cap ILP to keep generation tractable
ILP_PREFIX_SIZES = [5, 10, 15, 20]
...
def generate_scenario(object_count: int, bin_locs: List[List[float]], 
    ...
    ilp_prefixes, ilp_costs = compute_ilp_prefixes(
        obj_locs=obj_locs,
        obj_types=obj_types,
        bin_locs=bin_locs,
        start_pos=start_pos,
...
def solve_ilp_route(obj_locs: np.ndarray, obj_types: np.ndarray, bin_locs: List[List[float]],
                    start_pos: np.ndarray) -> Tuple[float, List[int]]:
    """
    Solve the order optimization ILP for the provided objects via CP-SAT
    `AddCircuit()` (replaces the previous CBC+MTZ formulation). Returns
    (total_cost, order_of_object_indices). Returns (None, None) on solver failure.
    """
    n = len(obj_locs)
    if n == 0:
        return 0.0, []
    scenario = {
        "objects": np.asarray(obj_locs, dtype=np.float32).tolist(),
        "types": np.asarray(obj_types, dtype=np.int32).tolist(),
        "bins": [list(b) for b in bin_locs],
        "start": np.asarray(start_pos, dtype=np.float32).tolist(),
    }
    try:
        cost, _ = solve_circuit_cold(scenario, max_objects=max(n + 2, 60))
    except RuntimeError:
        return None, None

    # solve_circuit_cold does not return the tour; reconstruct it by re-solving
    # with the same model and extracting arcs. For dataset generation we need
    # the order. Inline the extraction here to avoid a public API change.
    from gnn_ilp_circuit import build_cost_matrix, _build_circuit_model, _extract_circuit_solution, COST_SCALE
    from ortools.sat.python import cp_model as _cp_model

    costs, _pick_bin_cost, node_count, _start_idx = build_cost_matrix(scenario, max_objects=max(n + 2, 60))
    ...
    if status not in (_cp_model.OPTIMAL, _cp_model.FEASIBLE):
        return None, None
    active_arcs = _extract_circuit_solution(solver, arc_vars, node_count)
    successor = {i: j for (i, j) in active_arcs}
    order_objects: List[int] = []
    current = _start_idx
    for _ in range(node_count):
        nxt = successor.get(current)
        if nxt is None or nxt == _start_idx:
            break
        order_objects.append(nxt - 1)
        current = nxt
    return cost, order_objects


def compute_ilp_prefixes(obj_locs: np.ndarray, obj_types: np.ndarray, bin_locs: List[List[float]],
                         start_pos: np.ndarray) -> Tuple[Dict[str, List[int]], Dict[str, float]]:
    """
    Compute ILP solutions for configured prefixes. Skips when prefix exceeds ILP_MAX_OBJECTS.
    """
    ilp_orders: Dict[str, List[int]] = {}
    ilp_costs: Dict[str, float] = {}
    for prefix in ILP_PREFIX_SIZES:
        if prefix > len(obj_locs) or prefix > ILP_MAX_OBJECTS:
            continue
        cost, order = solve_ilp_route(
            obj_locs=obj_locs[:prefix],
            obj_types=obj_types[:prefix],
            bin_locs=bin_locs,
            start_pos=start_pos
        )
        if cost is None or order is None:
            continue
        ilp_orders[str(prefix)] = order
        ilp_costs[str(prefix)] = float(cost)
    return ilp_orders, ilp_costs
