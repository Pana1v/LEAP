...
def _draw_panel(ax, data, title, x_min, x_max):
    ...
    for m in methods:
        mean, std = data[m]
        std = std if std is not None and std > 0 else mean * 1e-6
        s = lognormal_samples(mean, std, n=500, rng=rng)
        sample_log.append(np.log10(np.clip(s, 1e-3, None)))

    parts = ax.violinplot(sample_log, positions=positions, vert=False,
                          showmeans=False, showextrema=False, widths=0.85)
    for body, m in zip(parts["bodies"], methods):
        body.set_facecolor(PALETTE.get(m, "#888"))
        body.set_edgecolor("k")
        body.set_alpha(0.85)
        body.set_linewidth(0.4)

    # Overlay published mean as a tick
    for i, m in enumerate(methods):
        mean, _ = data[m]
        ax.plot([math.log10(max(mean, 1e-3))], [i],
                marker="|", color="k", markersize=11, mew=1.4)
        ax.text(math.log10(max(mean, 1e-3)), i + 0.35,
                _time_fmt(mean, None), fontsize=6, ha="center", color="0.25")

    ax.set_yticks(positions)
    ax.set_yticklabels(methods, fontsize=8)
    ax.set_xlim(math.log10(x_min), math.log10(x_max))

    # Custom log x ticks
    decades = list(range(int(math.floor(math.log10(x_min))),
                         int(math.ceil(math.log10(x_max))) + 1))
    ax.set_xticks(decades)
    ax.set_xticklabels([_time_fmt(10 ** d, None) for d in decades], fontsize=7)
    ax.set_xlabel("Compute time per scenario (log scale)", fontsize=8.5)
    ax.set_title(title, fontsize=10, fontweight="bold")
    ax.grid(True, axis="x", which="both", linewidth=0.3, alpha=0.4)
...
def main():
    ...
    for N, ax in zip(Ns, axes):
        data = load_data(N)
        _draw_panel(ax, data, f"$N = {N}$", *panel_xrange[N])

    fig.suptitle(
        "Per-method runtime at $N \\in \\{10, 40, 200\\}$ — log-normal "
        "reconstruction from published moments",
        fontsize=20, fontweight="bold", y=1.04)
    fig.tight_layout()

    for ext in ("pdf", "png"):
        fig.savefig(OUT_DIR / f"fig_runtime_histogram.{ext}", dpi=200,
                    bbox_inches="tight", pad_inches=0.05)
    print(f"Saved fig_runtime_histogram.png to {OUT_DIR}")
    plt.close(fig)
