...
\begin{abstract}
Integer Linear Programming (ILP) solvers produce provably optimal pick-and-place (PAP) sequences but become intractable beyond ${\sim}30$--$40$ objects, while greedy heuristics scale trivially yet sacrifice 3--5\% in travel cost even when accounting for placement distance, and up to 10--40\% in constrained conveyor settings \cite{han2019pap}. We bridge this gap from two directions. First, we \emph{distill} the decision quality of an ILP solver into a lightweight Graph Neural Network (GNN) policy. The key enabler is a cycle-aware directed graph whose two cost-bearing edges encode the robot-to-object pick leg and object-to-bin place leg, closed by a bin-to-robot feedback edge that routes the robot's post-placement position back into the decision for the next step. Three semantically distinct node types (robot, object, bin) with type-encoded features are processed by a standard Graph Attention Network, capturing structural information discarded by flat sequence-to-sequence neural combinatorial optimisation (NCO) architectures. Trained via curriculum-based imitation learning on ILP-optimal prefixes with greedy fallback, the policy reaches a mean ILP gap of 1.81\% at $N{=}10$, improves 3.23\% over greedy on average at $N{=}40$, and wins on 98\% of 100 held-out scenarios ($p < 10^{-31}$, paired $t$-test). Second, we show that the GNN itself can \emph{accelerate} the exact solver: by using GNN logits to prune the arc space to $k$ candidates per node before passing the reduced problem to a CP-SAT circuit constraint solver, we achieve 140$\times$ speedup over the baseline CBC formulation at $N{=}200$ while preserving near-optimal solution quality (worst-case gap 0.062\%). This dual role---the GNN as both a standalone fast policy \emph{and} a search-space reducer for exact methods---extends near-optimal decision-making well beyond the reach of the solver alone.
\end{abstract}
...
We close this gap with two complementary contributions. First, we treat the ILP solver as a \emph{teacher} and a GNN as a \emph{student}: the exact solver produces provably optimal sequence prefixes offline, and the GNN learns to reproduce that reasoning at bounded, predictable inference cost. At deployment, the student runs alone, delivering ILP-quality sequences in milliseconds and, crucially, generalising to problem sizes ($N{=}100$, $N{=}200$) where the teacher is entirely intractable.

Second, we demonstrate that the trained GNN can \emph{accelerate} the exact solver itself. The key insight, inspired by the Neural Diving framework of Nair et al.\ \cite{nair2021neural}, is that the GNN's value lies not in providing an incumbent hint to the solver---which we show empirically provides negligible speedup---but in \emph{pruning the search space}. By using GNN logits to retain only the top-$k$ outgoing arcs per node, we reduce the number of decision variables from $O(N^2)$ to $O(Nk)$. Combined with a CP-SAT circuit constraint formulation \cite{ortools} that replaces the weak Miller--Tucker--Zemlin (MTZ) subtour elimination, this yields a 140$\times$ speedup over the baseline CBC solver at $N{=}200$, with worst-case optimality gap of 0.062\%. This makes the ILP \emph{tractable} at scales previously thought to require heuristic-only approaches.

Two structural insights enable the GNN distillation. First, PAP is \emph{not} symmetric TSP: the cost of picking object $i$ followed by object $j$ depends on which bin $i$ was deposited in, since the robot's position after placing $i$ is $b_{\tau(i)}$, not $o_i$. This asymmetry is precisely why Pointer Networks \cite{vinyals2015pointer} and attention-based models \cite{kool2019attention}, which embed all nodes as homogeneous 2D coordinates, require structural modification to handle PAP. Second, the pick-and-place action has an inherent \emph{cyclic} structure (robot$\to$object$\to$bin$\to$robot) that is naturally represented as a directed graph but is invisible to flat sequence encoders.
...
\begin{enumerate}
    ...
    \item A \textbf{GNN-guided ILP acceleration} technique that uses GNN logits to prune the arc space before exact solving. Combined with a CP-SAT Hamiltonian circuit formulation, this achieves up to 140$\times$ speedup over CBC at $N{=}200$, extending exact (or near-exact) ILP solutions to scales previously considered intractable.
...
\subsection{Neural Network--Guided Exact Solvers}

Nair et al.\ \cite{nair2021neural} introduced Neural Diving, which uses a neural network to produce partial variable assignments for mixed-integer programs, reducing the effective problem size before handing it to a traditional solver. Their key insight is that the network's value lies in \emph{shrinking the problem} rather than merely providing an incumbent bound. Khalil et al.\ \cite{khalil2016learning} learned branching policies for branch-and-bound, while Gasse et al.\ \cite{gasse2019exact} trained GNNs to imitate strong branching decisions. Our GNN-guided arc pruning follows the Neural Diving philosophy: rather than hinting the solver (which we show provides negligible benefit), we use GNN logits to eliminate low-probability arcs, reducing the decision variable count from $O(N^2)$ to $O(Nk)$.
...
%========================================================================
\section{GNN-Guided ILP Acceleration}\label{sec:ilp_accel}
%========================================================================

A natural question is whether the trained GNN can accelerate the ILP solver itself, enabling exact solutions at larger scales. We investigate several strategies and identify one that yields dramatic speedups.

\subsection{Baseline ILP Formulation}

The PAP sequencing problem is formulated as an asymmetric travelling salesman problem (ATSP) over $N{+}1$ nodes (the start position plus $N$ objects). The cost of arc $(i, j)$ is the distance from the bin assigned to object $i$ (or the start position, for $i{=}0$) to object $j$, plus the distance from object $j$ to its assigned bin. The baseline uses CBC \cite{ortools} with Miller--Tucker--Zemlin (MTZ) subtour elimination constraints \cite{mtz1960}, which introduce $O(N^2)$ big-$M$ constraints and $N$ auxiliary integer variables. The LP relaxation of MTZ is notoriously weak, producing large branch-and-bound trees.

\subsection{CP-SAT Circuit Formulation}

We replace the MTZ formulation with OR-Tools' CP-SAT solver using the native \texttt{AddCircuit()} global constraint. This constraint encodes Hamiltonian cycle detection via Tarjan's strongly connected components algorithm, performing $O(N{+}M)$ propagation (where $M$ is the arc count) at each search node---far tighter than MTZ's LP relaxation. The ATSP is reduced to a Hamiltonian cycle by adding zero-cost return arcs from every node back to the start node. Arc costs are scaled to integers (\texttt{COST\_SCALE} $= 10{,}000$) for CP-SAT's integer objective.

This single change yields a 5--7$\times$ speedup over CBC across all problem sizes, as the tighter propagation prunes the search tree more aggressively.

\subsection{Why Warm-Start Hinting Fails}\label{sec:hint_fail}

A natural approach is to provide the GNN's solution as a warm-start hint to the solver. We tested four variants:

\begin{enumerate}
\item \textbf{CBC \texttt{SetHint()}}: Provides the GNN solution as an incumbent bound. Result: 0.93--1.06$\times$ (neutral to slight slowdown). The hint gives CBC a feasible starting point, but does not reduce the branch-and-bound tree---the solver must still explore the full space to prove optimality.

\item \textbf{CP-SAT \texttt{AddHint()} on all arcs}: Hints all ${\sim}N^2$ arc variables (active arcs to 1, inactive to 0). Result: 0.51$\times$ (significant slowdown). The overhead of processing $N^2$ hints overwhelms CP-SAT's Large Neighbourhood Search workers.

\item \textbf{CP-SAT \texttt{AddHint()} on active arcs only}: Hints only the ${\sim}N$ active arcs (set to 1), leaving inactive arcs unhinted. Result: still slower than unhinted Circuit.

\item \textbf{Neural Diving two-phase} (inspired by \cite{nair2021neural}): Fix high-confidence arcs in Phase~1 to obtain a tight incumbent quickly, then solve the full problem in Phase~2 using this incumbent as a hint. Result: ${\sim}2\times$ speedup over CBC but slower than bare Circuit.
\end{enumerate}

The fundamental limitation is that hints provide only an \emph{incumbent bound}; they do not reduce the search space. The solver must still explore the full variable space to prove optimality. This motivates a different strategy: reduce the variable space \emph{before} invoking the solver.

\subsection{GNN-Guided Arc Pruning}

Instead of hinting, we use the GNN's per-step logits to \emph{eliminate} low-probability arcs from the problem entirely, following the spirit of Neural Diving's partial variable assignment \cite{nair2021neural} but applied to arc selection rather than variable fixing.

\subsubsection{Logit Extraction}
During a standard GNN rollout, we capture the raw logits $\ell_t \in \mathbb{R}^N$ at each step $t$. These logits encode the model's relative preference for each candidate object. We use them to rank outgoing arcs from each node.

\subsubsection{Arc Selection}
For each node $i$ in the ATSP graph, we retain only the top-$k$ outgoing arcs ranked by GNN logit score, plus the arcs used in the GNN's own solution. We also ensure each node has at least $k$ incoming arcs (using nearest-neighbour fallback by cost if needed). Return arcs to the start node are always included. This reduces the arc count from $N(N{+}1)$ to approximately $2Nk$.

\subsubsection{Pruned Circuit Solve}
The reduced arc set is passed to the CP-SAT Circuit solver, which now operates on $O(Nk)$ variables instead of $O(N^2)$. The GNN's active solution is hinted (active arcs only) to provide a good starting point. Algorithm~\ref{alg:pruned} summarises the procedure.

\begin{algorithm}[htbp]
\caption{GNN-Guided Arc Pruning for ILP Acceleration}
\label{alg:pruned}
\begin{algorithmic}[1]
\REQUIRE Scenario $S$, trained GNN $\pi_\theta$, neighbourhood size $k$
\ENSURE Near-optimal sequence, cost
\STATE Run GNN rollout on $S$; capture logits $\{\ell_t\}_{t=1}^{N}$ and sequence $\sigma$
\STATE Compute ATSP cost matrix $C \in \mathbb{R}^{(N+1)\times(N+1)}$
\STATE $\mathcal{A} \leftarrow$ arcs in GNN solution $\sigma$
\FOR{each node $i \in \{0, \dots, N\}$}
    \STATE Rank outgoing arcs $(i, j)$ by GNN logit score for $j$
    \STATE $\mathcal{A} \leftarrow \mathcal{A} \cup \{(i,j) : j \in \text{top-}k\text{ by logit}\}$
\ENDFOR
\STATE Ensure $\geq k$ incoming arcs per node (nearest-cost fallback)
\STATE Add return arcs $(i, 0)$ for all $i > 0$
\STATE Build CP-SAT model with \texttt{AddCircuit()} over $\mathcal{A}$
\STATE Hint active GNN arcs only
\STATE Solve; extract Hamiltonian path from circuit
\end{algorithmic}
\end{algorithm}

The key insight is that the GNN need not produce the \emph{optimal} solution---it only needs to assign high logit mass to arcs that \emph{include} the optimal arcs. As long as the optimal arc at each node appears in the top-$k$ candidates, the pruned solver will find it. This is a much weaker requirement than producing the optimal sequence directly, and explains why the pruning approach succeeds where standalone GNN inference only approximates.
...
A key operational advantage is \emph{predictable} inference time. ILP solve time grows exponentially with $N$ and frequently exceeds real-time budgets: at $N{=}40$, all instances exceed a 60\,s timeout with the baseline CBC formulation. GNN inference scales polynomially with graph size, yielding consistent median runtimes: 26\,ms at $N{=}10$, 125\,ms at $N{=}40$, and 960\,ms at $N{=}200$ (NVIDIA RTX~2050 GPU with CUDA synchronisation, 100 validation scenarios, 10 warmup passes). For comparison, Greedy runs in 0.02--8.3\,ms across the same range. Fig.~\ref{fig:timing_scaling} visualises this scaling behaviour.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\linewidth]{figures/paper/fig_timing_scaling.png}
\caption{Inference time scaling (log-scale). GNN inference grows polynomially with $N$, while the baseline CBC ILP is intractable beyond $N{\approx}40$. Greedy is fastest but sacrifices solution quality. The GNN-pruned ILP (Section~\ref{sec:ilp_accel}) extends exact solving to $N{=}200$, bridging the gap between GNN speed and ILP optimality. GNN runtime variability is low: std = 2.5\,ms (p95 = 31\,ms) at $N{=}10$; std = 6.0\,ms (p95 = 135\,ms) at $N{=}40$; std = 41\,ms (p95 = 1050\,ms) at $N{=}200$. At $N{=}10$, only 13 of 50 ILP instances solved within a 60\,s CPU timeout.}
\label{fig:timing_scaling}
\end{figure}

For $N{=}100$ and $N{=}200$, baseline CBC ILP is entirely intractable; the GNN provides the only near-optimal baseline, demonstrating the scalability payoff of solver distillation. Table~\ref{tab:results_200obj} confirms this: at $N{=}200$, the GNN improves on Greedy in 100\% of scenarios (100/100) with a mean improvement of 1.67\%.
...
\subsection{ILP Acceleration Results}\label{sec:ilp_accel_results}

Table~\ref{tab:ilp_speedup} reports the speedup achieved by the GNN-pruned Circuit solver (Section~\ref{sec:ilp_accel}) compared to the baseline CBC solver and the cold (unpruned) Circuit solver, using $k{=}15$ neighbours per node.

\begin{table}[htbp]
\caption{GNN-pruned ILP speedup ($k{=}15$ neighbours). ``vs CBC'' is speedup over the baseline CBC+MTZ solver. ``vs Circuit'' is the speedup attributable to GNN pruning alone (over unpruned CP-SAT Circuit). ``Max Gap'' is the worst-case optimality gap across all scenarios tested. ``Exact'' indicates how many scenarios achieved provably optimal solutions (zero gap).}
\label{tab:ilp_speedup}
\centering
\begin{tabular}{@{}rcccl@{}}
\toprule
$N$ & \textbf{vs CBC} & \textbf{vs Circuit} & \textbf{Max Gap} & \textbf{Exact} \\
\midrule
5   & 0.9$\times$   & 0.9$\times$  & 0\%     & 50/50 \\
10  & 1.4$\times$   & 0.6$\times$  & 0\%     & 50/50 \\
20  & 5.4$\times$   & 0.9$\times$  & 0\%     & 30/30 \\
40  & 8.1$\times$   & 1.4$\times$  & 0\%     & 20/20 \\
100 & 13.3$\times$  & 3.8$\times$  & 0.004\% & 10/10 \\
200 & 140$\times$   & 17.8$\times$ & 0.062\% & 1/5 \\
\bottomrule
\end{tabular}
\vspace{1mm}

\footnotesize At $N{\le}40$, the GNN-pruned solver finds provably optimal solutions (zero gap) on all instances. At $N{=}100$ the worst-case gap is 0.004\%; at $N{=}200$ it is 0.062\%. The speedup is dominated by the Circuit formulation at small $N$ and by GNN pruning at large $N$.
\end{table}

Several patterns emerge. At small scales ($N{\le}20$), the Circuit formulation alone accounts for most of the speedup, and GNN pruning provides marginal or negative benefit---the overhead of running the GNN and constructing the pruned arc set exceeds the solve-time reduction. At $N{=}40$, the GNN pruning begins to contribute (1.4$\times$ over cold Circuit), and its value grows superlinearly: at $N{=}200$, it provides 17.8$\times$ over cold Circuit, reducing ${\sim}40{,}200$ arc variables to ${\sim}3{,}200$.

The optimality gap remains negligible: zero at $N{\le}40$, 0.004\% at $N{=}100$, and 0.062\% at $N{=}200$. This means the pruned solver discards arcs that the optimal solution does not use, validating the hypothesis that the GNN assigns high logit mass to arcs near the optimum.

\textbf{Decomposition of speedup.} The 140$\times$ speedup at $N{=}200$ decomposes as follows: ${\sim}7.9\times$ from Circuit replacing MTZ (tighter propagation), and ${\sim}17.8\times$ from GNN arc pruning (fewer variables). The two improvements are multiplicative: Circuit provides tighter propagation on whatever arcs remain, and pruning reduces the number of arcs to propagate over.

\textbf{Comparison with hinting approaches.} Table~\ref{tab:hint_comparison} contrasts the pruning approach with the hinting strategies described in Section~\ref{sec:hint_fail}. The results confirm that the GNN's value for ILP acceleration lies in search-space reduction, not incumbent provision.

\begin{table}[htbp]
\caption{Comparison of GNN-assisted ILP strategies ($N{=}40$, speedup vs.\ CBC cold baseline). Only GNN arc pruning provides consistent speedup.}
...
\textbf{CP-SAT Circuit + GNN pruning ($k{=}15$)} & \textbf{8.1$\times$} \\
\bottomrule
\end{tabular}
\end{table}
...
\footnotesize The $N{=}10$ and $N{=}20$ ILP gaps are computed using pre-computed optimal costs cached in the dataset. $^{\ddag}$$N{=}40$ ILP is intractable for per-sample optima with the baseline CBC solver (exceeds 60\,s per instance); the GNN-pruned Circuit solver (Section~\ref{sec:ilp_accel_results}) can solve $N{=}40$ in seconds. GNN inference timing is reported in Fig.~\ref{fig:timing_scaling}.
\end{table}
...
We acknowledge several gaps. Generalisation to out-of-distribution scenarios (e.g., training on $N{=}20$, testing on $N{=}40$; training on corner bins, testing on random bin placements) has not been evaluated. The $N{=}200$ evaluation (Table~\ref{tab:results_200obj}) uses 100 scenarios but lacks paired significance testing. The imitation learning framework is subject to covariate shift \cite{ross2011dagger}; DAgger-style online data collection or RL fine-tuning could address this. The cost model uses Euclidean distance; kinematic or dynamic robot models would improve applicability to specific manipulator platforms. The GNN-pruned ILP solver (Section~\ref{sec:ilp_accel}) produces near-optimal but not provably optimal solutions at $N{\ge}100$; the worst-case gap of 0.062\% at $N{=}200$ is negligible in practice but the guarantee is weaker than exact ILP. The choice of $k{=}15$ neighbours was not extensively tuned; smaller $k$ would be faster but risk excluding optimal arcs at higher rates.

%========================================================================
\section{Conclusion}\label{sec:conclusion}
%========================================================================

We have presented a method for distilling the decision quality of an exact ILP solver into a lightweight GNN policy for robotic pick-and-place sequencing, and shown that the same GNN can accelerate the exact solver itself.

The key architectural enabler for distillation is the cyclic star graph that explicitly represents the causal structure of pick-and-place actions: robot, object, and bin nodes connected by directed edges encoding the full pick-place-feedback cycle. Type information is encoded in node features and processed by a standard GAT. This structural representation captures information that flat sequence-to-sequence NCO architectures fundamentally discard.

Trained via curriculum-based imitation learning on ILP-optimal prefixes with greedy fallback, the policy reaches 1.81\% mean ILP gap at $N{=}10$ (with exact matches on a subset of instances), 98\% win-rate vs.\ greedy at $N{=}40$ (98/100 scenarios, $p < 10^{-31}$), and scales to $N{=}200$ where exact ILP is entirely intractable with baseline solvers.

Beyond standalone inference, we demonstrated that the GNN's logits can guide arc pruning for the exact solver, reducing $O(N^2)$ decision variables to $O(Nk)$. Combined with a CP-SAT circuit constraint formulation, this achieves 140$\times$ speedup over the baseline CBC solver at $N{=}200$ with worst-case optimality gap of 0.062\%. The pruning approach succeeds where traditional warm-start hinting fails: the GNN's value lies in \emph{shrinking the problem}, not in providing an incumbent bound. This dual role---the GNN as both a fast standalone policy and a search-space reducer for exact methods---extends near-optimal sequencing beyond the computational reach of the solver alone.

Future work includes: DAgger or RL fine-tuning to address covariate shift, extension to online conveyor-belt settings with temporal constraints, kinematic cost models for specific manipulator platforms \cite{han2019pap} with obstacle-aware edge costs derived from motion planners such as dynamic PRM reshaping \cite{ojha2025obstacle}, cross-distribution generalisation experiments, adaptive $k$ selection for the arc pruning (using GNN confidence to adjust neighbourhood size per node), and validation on physical robotic sorting systems.
...
\bibitem{nair2021neural} V.~Nair, S.~Bartunov, F.~Gimeno, I.~von Glehn, P.~Lichocki, I.~Lobov, B.~O'Donoghue, N.~Sonnerat, C.~Tjandraatmadja, P.~Wang, R.~Addanki, T.~Hapuarachchi, T.~Keck, J.~Keeling, P.~Kohli, I.~Ktena, Y.~Li, O.~Vinyals, and Y.~Zwols, ``Solving mixed integer programs using neural networks,'' \emph{arXiv preprint arXiv:2012.13349}, 2021.

\bibitem{khalil2016learning} E.~B.~Khalil, P.~Le Bodic, L.~Song, G.~Nemhauser, and B.~Dilkina, ``Learning to branch in mixed integer programming,'' in \emph{AAAI}, pp.~724--731, 2016.

\bibitem{gasse2019exact} M.~Gasse, D.~Ch\'{e}telat, N.~Ferroni, L.~Charlin, and A.~Lodi, ``Exact combinatorial optimization with graph convolutional neural networks,'' in \emph{NeurIPS}, vol.~32, 2019.
